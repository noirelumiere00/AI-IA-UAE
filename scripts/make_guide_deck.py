...
_note(s, "AiLaは『毎朝Slackに今日やることが届くメール秘書』。今日は5分で使い方を説明します。"
         "やることはGoogle連携（＋任意でSlack連携）のリンクをタップするだけ、という所をゴールに。")
...
# ── 3. はじめ方 ───────────────────────────────────────────────────────────────
content_slide(
    "はじめ方 ― 連携はリンクをタップするだけ（所要1分）",
    [("① 管理者がSlackに貼った「メール連携リンク」をタップ → 自分のGoogle（Gmail・カレンダー）を許可", 0),
     ("②（任意）「Slack連携リンク」もタップ → 自分のSlackを許可（未返信メンションも拾う）", 0),
     ("③ 「✅ 連携が完了しました」と出れば完了 → 翌朝から自動で届く", 0),
     ("見えるのは自分のメール／自分宛メンションだけ。AiLaが勝手に送信・投稿することはありません", 2)],
    "実演できるなら管理者がSlackに貼った連携リンクを画面で見せる。『リンクをタップするだけ・1分・自分の分だけ』を強調。"
    "Slackは任意で、後から足してもよい。ここさえ越えれば翌朝から価値が出る。", kicker="STEP")

# ── 4. 読み方 ─────────────────────────────────────────────────────────────────
content_slide(
    "メールサマリーの読み方（上から優先度順）",
    [("🔔 未返信 ― 今日中に … 重要なのに返信が滞留（最優先で対応）", 0),
     ("🔔 未返信に Slackの未返信メンションも合流 … 【メンション】〔#チャンネル〕 @相手 抜粋 — N営業日未返信", 0),
...
content_slide(
    "まとめ ― まずは連携リンクをタップから",
    [("やることは連携リンクをタップするだけ（Google＋任意でSlack）", 0),
     ("毎朝、未返信ゼロ・予定の見落としゼロへ", 0),
     ("困ったら：届かない→連携リンクの再タップ ／ ボタン無反応→数秒待って再操作", 0),
     ("お問い合わせ：小俣（AiLa 管理者）", 0)],
    "クロージング：『今日この場で連携リンクをタップしてみましょう』と行動を促す。"
    "Slackは任意で後追いでもOK。1〜2週間使って感想をください、で締める。", kicker="まとめ")
